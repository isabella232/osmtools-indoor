...
building.room = function(id, coords) {
  ...
  this.modal = function() {
    ...
    if (this.opening_hours != null) {
      window_text += '<h5>Opening hours</h5>\n';
      ...
      for (var i = 0; i <= 6; i++) {
        window_text += '<li><span>' + $.format.date(from, 'dd/MM/yyyy') + '</span>';
        ...
        if (!$.isEmptyObject(opening_hours)) {
          $.each(opening_hours, function(key, value) {
            window_text += '<span class="open">'
                    + $.format.date(value[0], 'hh:mm a')
                    + ' - '
                    + $.format.date(value[1], 'hh:mm a')
                    + '</span>';
          });
        } else {
          window_text += '<span class="closed">Closed</span>\n';
        }
        window_text += '</li>\n';
